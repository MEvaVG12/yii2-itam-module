...
<div class="workstation-form">
    ...
    <?= $form->field($model->assetWorkstation, 'id_os')->dropDownList(ArrayHelper::map(Os::find()->orderBy(['name' => SORT_ASC])->all(), 'id', 'name'), ['prompt' => '--']) ?>
    <?= $form->field($model->assetWorkstation, 'id_os_license')->dropDownList(ArrayHelper::map(OsLicense::find()->where(['id_os' => $model->assetWorkstation->id_os])->orderBy(['date' => SORT_DESC])->all(), 'id', 'key'), ['prompt' => '--']) ?>
    ...
    <?= $form->field($model->assetWorkstation, 'id_office_suite_license')->dropDownList(ArrayHelper::map(OfficeSuiteLicense::find()->where(['id_office_suite' => $model->assetWorkstation->id_office_suite])->orderBy(['date' => SORT_DESC])->all(), 'id', 'key'), ['prompt' => '--']) ?>
